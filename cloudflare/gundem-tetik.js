...
//
// GitHub'ın kendi zamanlayıcısı bu depoda saatlerce gecikebildiği için günlük koşuyu tam saatinde
...
//   3. Settings → Triggers → Cron Triggers →  50 4 * * *
//        (UTC; = her gün 07:50 TR. Koşu ~4 dk sürdüğünden bülten ~08:00'de gelen kutusuna düşer.)
//
// Hafta içi (TR saatiyle Pzt–Cum) bülten tüm listeye gönderilir; hafta sonu yalnız site güncellenir.
...
async function tetikle(env) {
  ...
  const r = await fetch(DISPATCH_URL, {
    ...
    body: JSON.stringify({ ref: "main", inputs: { bulten } }),
  });
  if (!r.ok) throw new Error(`GitHub workflow_dispatch başarısız: HTTP ${r.status}`);
  console.log(`gundem.yml tetiklendi (bülten: ${bulten})`);
}
